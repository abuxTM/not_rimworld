...
void pawn_update(Pawn* pawn) {
  ...
  if (pawn->is_controled) {
    ...
    if (global.key_state[SDLK_a] % MAX_KEYS) { pawn->dir.x -= pawn->speed; pawn->flip = SDL_FLIP_HORIZONTAL; }
    if (global.key_state[SDLK_d] % MAX_KEYS) { pawn->dir.x += pawn->speed; pawn->flip = SDL_FLIP_NONE; }
    if (global.key_state[SDLK_LSHIFT % MAX_KEYS]) pawn->speed /= 2;
    if (global.key_state[SDLK_c] % MAX_KEYS) inventory_use_item(pawn->inventory, 0);
...
void pawn_render(Pawn* pawn) {
  ...
  if (pawn->texture) SDL_RenderCopyEx(global.renderer, pawn->texture, NULL, &camera_rect, 0, NULL, pawn->flip);
